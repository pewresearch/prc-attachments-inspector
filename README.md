...
All routes are under the `prc-api/v3` namespace (registered via the `prc_api_endpoints` filter).
...
| `prc_api_endpoints` | Filter | Appends the panel and report REST routes to the PRC API endpoint registry |
...
- `prc-platform-core` — required plugin; provides the `prc_api_endpoints` filter used to register REST routes
...
Run Playwright tests (requires Playground to be running):

```bash
npm run playground:start
npm run test -w @prc/attachments-inspector
```
